    ...
    <section className="py-20 px-4 bg-white">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-bold text-gray-800 mb-6 font-playfair">Redefinimos el envejecimiento activo</h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {features.map((feature, index) => (
            <Card key={index} className="group hover:shadow-lg transition-all duration-300 border border-gray-50 hover:border-orange-100 bg-gradient-to-b from-white to-gray-25 hover:to-white">
              <CardContent className="p-8 h-full flex flex-col">
                <div className="flex items-center mb-6">
                  <div className="w-14 h-14 bg-gradient-to-br from-orange-50 to-orange-25 rounded-xl flex items-center justify-center shadow-sm">
                    <feature.icon className="w-7 h-7 text-orange-400" />
                  </div>
                  <span className="ml-4 w-1 h-8 bg-orange-200 rounded-full"></span>
                </div>
                <h3 className="text-xl font-bold text-gray-800 mb-3 font-playfair">{feature.title}</h3>
                <p className="text-gray-500 leading-relaxed mb-4">{feature.description}</p>
                <div className="mt-auto pt-4 border-t border-gray-50">
                  <span className="text-sm font-medium text-orange-400 hover:text-orange-500 transition-colors cursor-pointer">
                    Conocer más →
                  </span>
                </div>
              </CardContent>
